...
static int checkArguments(struct rules_actions_t *obj) {
	...
	if(jto == NULL) {
		logprintf(LOG_ERR, "write action is missing a \"TO\"");
		return -1;
	}
	if(jmode == NULL) {
		logprintf(LOG_ERR, "write action is missing a \"MODE\"");
		return -1;
	}
	if(jtext == NULL) {
		logprintf(LOG_ERR, "write action is missing a \"TEXT\"");
		return -1;
	}
	...
	if(nrvalues != 1) {
		logprintf(LOG_ERR, "write action \"TO\" only takes one argument");
		return -1;
	}
	...
	if(nrvalues != 1) {
		logprintf(LOG_ERR, "write action \"MODE\" only takes one argument");
		return -1;
	}
	jval = json_find_element(jvalues, 0);
		
	if(jval->tag != JSON_STRING || ((strcmp(jval->string_, "new") != 0) && (strcmp(jval->string_, "append") != 0))) {
		logprintf(LOG_ERR, "write action \"MODE\" argument must be either \"new\" or \"append\"");
		return -1;
	}
	...
	if(nrvalues != 1) {
		logprintf(LOG_ERR, "write action \"TEXT\" only takes one argument");
		return -1;
	}
	return 0;
}
		...
		if(jvalues1 != NULL && jvalues2 != NULL && jvalues3 != NULL) {
			...
			if(jval1 != NULL && jval2 != NULL  && jval3 != NULL &&
			 jval1->tag == JSON_STRING && jval2->tag == JSON_STRING && jval3->tag == JSON_STRING) {
				...
				logprintf(LOG_NOTICE, "write action \"MODE\" must be either \"new\" or \"append\"");
				}
				if(mode != NULL && (fp = fopen(file, mode)) != NULL){
					if(fprintf(fp, "%s%s", jval3->string_,"\n") > 0) {
						logprintf(LOG_DEBUG, "write action to file \"%s\" with text \"%s\" succeeded", file, jval3->string_);
					}
					fclose(fp);
				} else {
					logprintf(LOG_NOTICE, "write action couldn't open \"%s\" with mode \"%s\"", jval1->string_, jval2->string_);			
				}
				FREE(file);
				FREE(mode);
			} else {
				logprintf(LOG_NOTICE, "write action couldn't operate due to missing or bad parameters");
			}
		}
	}
...
__attribute__((weak))
#endif
void actionWriteInit(void) {
	event_action_register(&action_write, "write");

	options_add(&action_write->options, 'a', "TO", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_STRING, NULL, NULL);
	options_add(&action_write->options, 'b', "MODE", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_STRING, NULL, NULL);
	options_add(&action_write->options, 'c', "TEXT", OPTION_HAS_VALUE, DEVICES_VALUE, JSON_STRING, NULL, NULL);

	action_write->run = &run;
	action_write->checkArguments = &checkArguments;
}
